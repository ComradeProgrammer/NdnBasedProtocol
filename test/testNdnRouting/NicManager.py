    ...
    def assignIP(self):
        ...
        for link in self.net.links:
            ...
            if not host1 in self.hostToInterfaces:
                self.hostToInterfaces[host1] = [
                    (str(link.intf1), link.intf1.IP())]
            else:
                self.hostToInterfaces[host1].append(
                    (str(link.intf1), link.intf1.IP()))
            host2 = link.intf2.node.name
            if not host2 in self.hostToInterfaces:
                self.hostToInterfaces[host2] = [
                    (str(link.intf2), link.intf2.IP())]
            else:
                self.hostToInterfaces[host2].append(
                    (str(link.intf2), link.intf2.IP()))
