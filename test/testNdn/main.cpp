...
class FakeProtocol:public NdnProtocolPlus{
    public:
    FakeProtocol(std::shared_ptr<NdnProtocol> _protocol){
        protocol=_protocol;
    }
    virtual void onReceiveNdnPacket(int interfaceIndex, MacAddress sourceMac, std::shared_ptr<NdnPacket> packet)override{
        LOGGER->INFOF("FakeProtocol:packet received from interface %d,sourcemac %s, packet %s",interfaceIndex,sourceMac.toString().c_str(),packet->toString().c_str());
