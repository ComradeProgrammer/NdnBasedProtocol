...
NICManager::NICManager(std::shared_ptr<Logger> _logger) { logger = Logger::getDefaultLoggerIfNull(_logger); }

void NICManager::flush() {
    nicVectorCache.clear();
    nicMapCache.clear();
...
void NICManager::startMonitor() {
    thread monitorDaemon([this]() -> void {
        while (1) {
            ...
            // debug
            // logger->INFO("old");
            // for (auto i : oldNicMap) {
            //     logger->INFO(i.second.toString());
            // }
            // logger->INFO("new");
            // for (auto i : nicMapCache) {
            //     logger->INFO(i.second.toString());
            // }

            // currently we only consider that some NIC may go down. we don't considerate the possibility that a NIC get
            // deleted or created.
