...
class MyForwardStrategy : public ForwardDataStrategyBase {
   public:
    virtual std::vector<std::pair<int, MacAddress>> operator()(int interfaceIndex, MacAddress sourceMac, std::shared_ptr<NdnData> data,
        ...
        if (splits.size() > 1 && splits[1] == "rt" && interfaceIndex != NDN_ROUTING) {
