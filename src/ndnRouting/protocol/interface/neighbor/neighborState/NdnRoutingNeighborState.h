...
enum NeighborStateType { DOWN_STATE = 0, INIT_STATE = 1, EXCHANGE_STATE = 2, LOADING_STATE = 4, FULL_STATE = 8 };
...
class NdnRoutingNeighborState {
   public:
    NdnRoutingNeighborState(NdnRoutingNeighbor* _neighbor, NdnRoutingInterface* _interface,
                            std::shared_ptr<Logger> _logger = nullptr)
        : neighbor(_neighbor), interface(_interface) {
