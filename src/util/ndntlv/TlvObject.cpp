...
TlvObject::TlvObject(uint64_t _type, uint32_t data) {
    ...
    TlvObject(_type, 4, reinterpret_cast<char*>(&tmp));
}

TlvObject::TlvObject(uint64_t _type, uint16_t data) {
    uint16_t tmp = htons(data);
    TlvObject(_type, 2, reinterpret_cast<char*>(&tmp));
}

TlvObject::TlvObject(uint64_t _type, string data) {
    TlvObject(_type, data.size(), data.c_str());
}
...
const TlvObject TlvObject::decode(char* rawData){
    int ptr=0;
    //read type
    ...
    return TlvObject(type,length,rawData);
