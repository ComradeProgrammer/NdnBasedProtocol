...
NdnTransmitter::NdnTransmitter(std::shared_ptr<Logger> log ){
    ...
    for(int i=0;i<allNic.size();i++){
        ...
        logger->INFO("NdnTransmitter: detected "+allNic[i].getName()+"at unterface "+to_string(allNic[i].getInterfaceID())+", mac address"+allNic[i].getMacAddress().toString());
    }
...
void NdnTransmitter::setOnReceivePacket(function<void(int interfaceIndex,MacAddress sourceMac, shared_ptr<NdnPacket> packet)>_handler){
    handler=_handler;
}
...
void NdnTransmitter::send(int interfaceIndex,MacAddress destination, std::shared_ptr<NdnPacket>packet){
    ...
    auto ethernetPacket=make_shared<EthernetPacket>(destination,source,NDN_PROTOCOL,rawNdnPacket.second.get(),rawNdnPacket.first,logger);
    ...
    if(res<0){
        ...
        logger->INFO("NdnTransmitter::send: sendout packet to "+destination.toString());
    }
}
...
void NdnTransmitter::listen(){
    while(1){
        ...
        handler(res.first,res.second->getHeader().getDestinationMacAddress(),packet);
    }
}
