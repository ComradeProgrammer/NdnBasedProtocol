...
class NicManager {
   public:
    ...
    /**
     * @brief start a new thread to polling the status of all nics and call notifyObservers once differences between new
     * results and old results are found.
     */
    virtual void startMonitor() = 0;
    /**
     * @brief register an observer to monitor events on one or all interfaces. Once an event happens, all observers
     * monitoring this interface will be notified by calling its onEventHappen() method.
     *
     * @param observer The object which want to monitor the interface. Should have implemented NicObserverInterface
     * interface
